...
export default function ProductReveal() {
  const activeProductId = useCharmoraStore((s) => s.activeProductId);
  const product = getProduct(activeProductId);
  const hasSecondAngle = product.images.length > 1;
  ...
  const trust = [
    { Icon: ShieldCheck, label: product.metal === "gold" ? "14k Gold Vermeil" : "Sterling Silver" },
  ...
  return (
    <section id="reveal" className="relative overflow-hidden bg-cream py-24 md:py-36">
      ...
      <div className="relative mx-auto max-w-7xl px-6 md:px-10">
        <Reveal key={`${product.id}-label`} className="mx-auto max-w-xl text-center">
          <SectionLabel>Product Reveal</SectionLabel>
          <h2 className="mt-6 font-display text-4xl font-light text-ink md:text-5xl">
            {product.name} <span className="italic text-gold-dark">— {product.meaningLabel}</span>
          </h2>
        ...
        <div className="relative mx-auto mt-16 flex max-w-3xl items-center justify-center">
          ...
          {hasSecondAngle ? (
            ...
            >
              <motion.div style={{ opacity: inverseCrossfade }} className="absolute inset-0">
                <Image
                  ...
                  alt={`The Charmora ${product.name}, studio shot`}
                  fill
                  draggable={false}
              ...
              <motion.div style={{ opacity: crossfade }} className="absolute inset-0">
                <Image
                  ...
                  alt={`The Charmora ${product.name}, alternate angle`}
                  fill
                  draggable={false}
                  sizes="(max-width: 640px) 90vw, 480px"
                  className="object-contain drop-shadow-[0_30px_60px_rgba(35,28,24,0.18)]"
            ...
            <motion.div
              ...
              className="relative z-10 aspect-[4/5] w-full max-w-sm sm:max-w-md"
            >
              <Image
                src={product.images[0]}
                alt={`The Charmora ${product.name}`}
                fill
                sizes="(max-width: 640px) 90vw, 480px"
                className="object-contain drop-shadow-[0_30px_60px_rgba(35,28,24,0.18)]"
                priority
